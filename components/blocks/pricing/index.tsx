	...
	return (
		...
		>
			{plan.highlighted && (
				<BorderTrail
					style={{
						/* DESIGN-CRITICAL: border trail glow uses hardcoded rgb for cross-browser animation */
						boxShadow:
					...
					<div key={index} className="flex items-center gap-2">
						...
						<TooltipProvider>
							<Tooltip delayDuration={0}>
								<TooltipTrigger render={<p className={cn(
                                											feature.tooltip &&
                                												'cursor-pointer border-b border-dashed',
                                										)} />}>{feature.text}</TooltipTrigger>
								{feature.tooltip && (
									<TooltipContent>
										<p>{feature.tooltip}</p>
									</TooltipContent>
			...
			>
				<Button className="w-full" variant={plan.highlighted ? 'default' : 'outline'} render={<a href={plan.btn.href} />} nativeButton={false}>{plan.btn.text}</Button>
			</div>
		</div>
	);
}
